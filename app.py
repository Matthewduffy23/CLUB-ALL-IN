...
LEAGUE_STRENGTHS = {
    ...
    "Portugal 4":25.00,"USA 3":22.55,"Scotland 3":20.00,"Sweden 4":20.00,"Switzerland 4":20.00,
    "Switzerland 3":35.00,
...
COUNTRY_TO_REGION = {
    ...
    "Kazakhstan":"Europe","Israel":"Europe","Turkey":"Europe","Australia":"Oceania",
...
with st.sidebar:
    ...
    # Filtered to each dropdown's actual file type, not one shared list —
    # previously both selectors defaulted to whatever file sorted first
    # overall, so the same file landed as the default in BOTH "Team Stats
    # CSV" and "Player CSV" regardless of type.
    ...
    # 2021-22. _season_key reads the year from either naming pattern
    ...
    csv_candidates=sorted(Path.cwd().glob("*.csv"),key=lambda c:_season_key(c.name),reverse=True)
    _team_csv_candidates=[c for c in csv_candidates if "WORLDTEAMS" in c.name]
    if _team_csv_candidates:
        _csv_names=[c.name for c in _team_csv_candidates]
    ...
    _player_csvs=[c.name for c in csv_candidates if "WORLDTEAMS" not in c.name]
    if _player_csvs:
    ...
    show_goals     =st.toggle("Goals",False,key="sq_goals")
    show_assists   =st.toggle("Assists",False,key="sq_assists")
    show_roles     =st.toggle("Role scores",False,key="sq_roles")
...
def render_pro_layout_v2(team_players_df: pd.DataFrame, df_sc, df_full: pd.DataFrame = None):
    ...
    for rank_i, (_, row) in enumerate(df_team.iterrows(), start=1):
        ...
        cy  = pd.to_datetime(row.get("Contract expires"), errors="coerce", dayfirst=True)
        ...
        if _tr_df is None or _tr_df.empty:
            ...
            if _tr_filtered.empty:
                ...
                if st.session_state.get("_tr_img_bytes"):
                    st.image(st.session_state["_tr_img_bytes"], width="stretch")
                    st.download_button(
...
# TEAM ARCHETYPE MAP
    ...
    _arch_buf.seek(0)
    plt.close(_arch_fig)
    st.image(_arch_buf, width="stretch")
    st.download_button(
    ...
    for _rr1_tab, _rr1_rk in zip(_rr1_tabs, _RR_TAB_KEYS):
        with _rr1_tab:
            ...
            with _rr1_c2:
                ...
                st.pyplot(_rr1_fig, width="stretch")
        ...
        with _rr2_tab:
            ...
            with _rr2_c2:
                st.markdown(f'<p style="color:#9ca3af;font-size:12px;margin-bottom:0;">{_rr2_subtitle}</p>',
                            unsafe_allow_html=True)
                st.pyplot(_rr2_fig, width="stretch")
