...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        'Build a custom forecast template — type 0–100 for any metric to generate a split radar. '
        'Left = Team style &nbsp;·&nbsp; Right = Role profile. '
        'Min 4 metrics per side, add or replace freely.</p>',
        unsafe_allow_html=True,
    )
    ...
    _PV_EXTRA_OPTIONS = [
        "Aerial Volume", "Attacking Contribution", "Attacking Territory", "Ball Carrying",
        "Build Up Speed", "Creativity", "Crosses", "Deeper Playmaking", "Defensive Volume",
        "Goal Output", "Goal Threat", "High Press", "Line Height", "Long Balls",
        "Opportunities", "Pass Verticality", "Pass Volume", "Possession", "Passes",
        "Pressing", "Progression Volume", "Retention", "xG", "xGA",
        "Aerial Requirement", "Shot Volume", "Counter Press", "Width",
        "Vertical Runs", "Box Entries", "Set Piece Threat", "Transition Speed",
        "Off Ball Movement", "Compactness", "Pressing Intensity", "Direct Play",
        "Sweeping", "Shots Faced", "Long Pass Volume", "Short Pass Volume",
        "Distribution", "Claim Rate", "Command of Area",
    ]

    def _pv_metric_editor(side_label, defaults, section_key, role_key, color_hint):
        """
        Renders add/remove/replace UI for one side of the projected view.
        Returns list of (label, percentile) tuples.
        """
        state_key = f"_pv_metrics_{section_key}_{role_key}"
        if state_key not in st.session_state:
            st.session_state[state_key] = list(defaults)

        metrics = st.session_state[state_key]

        st.markdown(
            f'<p style="color:{color_hint};font-weight:800;font-size:13px;'
            f'letter-spacing:0.08em;margin-bottom:4px;">{side_label.upper()}</p>',
            unsafe_allow_html=True,
        )

        # Add metric row
        extra_opts = sorted(set(_PV_EXTRA_OPTIONS) - set(metrics))
        add_cols = st.columns([3, 1])
        with add_cols[0]:
            new_metric = st.selectbox(
                "Add metric",
                ["— select to add —"] + extra_opts,
                key=f"_pv_add_{section_key}_{role_key}",
                label_visibility="collapsed",
            )
        with add_cols[1]:
            if st.button("＋", key=f"_pv_addbtn_{section_key}_{role_key}",
                         help="Add metric", use_container_width=True):
                if new_metric != "— select to add —" and new_metric not in metrics:
                    st.session_state[state_key].append(new_metric)
                    st.rerun()

        # Per-metric: value input + replace + remove
        result = []
        to_remove = None
        to_replace = {}

        for i, m in enumerate(metrics):
            row = st.columns([2, 1, 2, 0.5])
            with row[0]:
                st.markdown(
                    f'<p style="color:#e5e7eb;font-size:12px;margin:0;padding-top:6px;">{m}</p>',
                    unsafe_allow_html=True,
                )
            with row[1]:
                val = st.number_input(
                    "pct", min_value=0, max_value=100, value=50, step=1,
                    key=f"_pv_val_{section_key}_{role_key}_{i}",
                    label_visibility="collapsed",
                )
            with row[2]:
                replace_opts = sorted(set(_PV_EXTRA_OPTIONS) - set(metrics) | {m})
                replace_sel = st.selectbox(
                    "Replace",
                    ["— replace —"] + [x for x in replace_opts if x != m],
                    key=f"_pv_rep_{section_key}_{role_key}_{i}",
                    label_visibility="collapsed",
                )
                if replace_sel != "— replace —":
                    to_replace[i] = (m, replace_sel)
            with row[3]:
                if len(metrics) > 4:
                    if st.button("✕", key=f"_pv_rem_{section_key}_{role_key}_{i}",
                                 use_container_width=True):
                        to_remove = i
            result.append((m, int(val)))

        if to_remove is not None:
            st.session_state[state_key].pop(to_remove)
            st.rerun()
        if to_replace:
            for idx, (old, new) in to_replace.items():
                st.session_state[state_key][idx] = new
            st.rerun()
    ...
    for _pv_tab, _pv_rk in zip(_pv_tabs, _PV_DEFAULTS.keys()):
        with _pv_tab:
            ...
            with _pv_left:
                st.markdown(
                    '<p style="color:#9ca3af;font-size:12px;margin-bottom:6px;">'
                    'Type 0–100 for each metric. Use ＋ to add, ✕ to remove (min 4), '
                    'or replace via dropdown.</p>',
                    unsafe_allow_html=True,
                )
                _pv_team_vals = _pv_metric_editor(
                    "Team", _pv_cfg["team"], "team", _pv_rk, "#60a5fa"
                )
                st.markdown('<div style="height:12px;"></div>', unsafe_allow_html=True)
                _pv_role_vals = _pv_metric_editor(
                    "Role", _pv_cfg["role"], "role", _pv_rk, "#f472b6"
                )
            ...
            with _pv_right:
                _pv_t_labels = [x[0] for x in _pv_team_vals]
                _pv_t_pcts   = [x[1] for x in _pv_team_vals]
                _pv_r_labels = [x[0] for x in _pv_role_vals]
                _pv_r_pcts   = [x[1] for x in _pv_role_vals]

                if len(_pv_t_labels) < 4 or len(_pv_r_labels) < 4:
                    st.warning("Need at least 4 metrics per side to render the chart.")
                else:
                    _pv_fig = _rr_split_polar_fig(
                        _pv_t_labels, _pv_t_pcts,
                        _pv_r_labels, _pv_r_pcts,
                    )
                    # Overlay custom label at bottom centre
                    _pv_fig.text(
                        0.5, 0.01, _pv_label,
                    ...
                    st.markdown(
                        ...
                        f'{_pv_cfg["title"]} — Projected View</p>',
                        unsafe_allow_html=True,
                    )
                    st.pyplot(_pv_fig, use_container_width=True)

                    _pv_buf = _BytesIO_rr()
                    _pv_fig.savefig(
                        _pv_buf, format="png", dpi=300,
                        bbox_inches="tight", facecolor=_pv_fig.get_facecolor()
                    )
                    _pv_buf.seek(0)
                    st.download_button(
                        f"⬇️ Download {_pv_cfg['title']} projected radar",
