...
# ═══════════════════════════════════════════════════════════════════════════════
# TEAM PLAYER RANKINGS  —  paste after the Pro Layout section in team_squad_app.py
# ═══════════════════════════════════════════════════════════════════════════════
# v2 changes
#   1. Bar filled to league-best for that metric/score (not team-best)
#   2. Silhouette fallback image when photo not fetched
#   3. All position tokens shown below player name  (e.g. "CB, LCB")
#   4. New "Best Role" rank mode — ranks by each player's highest role score,
#      shows role name instead of positions below the name in the image
# ═══════════════════════════════════════════════════════════════════════════════

import math as _math

# ─────────────────────────────────────────────────────────────────────────────
# Silhouette fallback (loaded once, cached)
# ─────────────────────────────────────────────────────────────────────────────
_SILHOUETTE_URL = "https://cdn-icons-png.flaticon.com/512/149/149071.png"

@st.cache_data(show_spinner=False)
def _tr_load_silhouette() -> np.ndarray | None:
    try:
        img = load_remote_img(_SILHOUETTE_URL)
        return img
    except Exception:
        return None
...
def _build_team_rank_df(
    ...
    """
    Returns a per-player DataFrame with:
      · raw metric values
      · _pct_{metric}  — percentile vs league+position pool (0–100)
      · _lgbest_{metric} — league-best raw value for that metric+position (for bar scaling)
      · Complete Score, Impact Score
      · _role_{name}   — role scores
      · _best_role_name, _best_role_score — top role for "Best Role" mode
      · _all_positions — all position tokens joined, e.g. "CB, LCB"
    """
    df_team = pd.read_json(io.BytesIO(team_players_bytes))
    df_all  = pd.read_json(io.BytesIO(all_players_bytes))

    for df in [df_team, df_all]:
        if "_ftok" not in df.columns:
            df["_ftok"] = df["Position"].apply(_tok)

    df_team = df_team[
        pd.to_numeric(df_team.get("Minutes played", 0), errors="coerce").fillna(0) >= min_mins
    ].copy()
    ...
    for pk, toks in _POS_TOK_SET.items():
        ...
        for m in all_metric_cols:
            if m in sub.columns:
                ...
                _lg_bests[(pk, m)] = float(s.max()) if not s.empty else np.nan
    ...
    for _, row in df_team.iterrows():
        ...
        # All displayed positions (deduplicated, comma-joined)
        all_toks = [t.strip().upper() for t in pos_str.split(",") if t.strip()]
        ...
        rec = {
            ...
            "Position":       pos_str,
            "_all_positions": ", ".join(unique_toks),
            "_ftok":          tok,
            "_pos_key":       pk,
        ...
        for m in _all_pct_metrics:
            ...
            if m in df_all.columns:
                ...
                if pd.notna(val) and not ref_s.empty:
                    rec[f"_pct_{m}"] = float((ref_s < val).mean() * 100 + (ref_s == val).mean() * 50)
                    rec[f"_lgbest_{m}"] = float(ref_s.max())
                else:
                    rec[f"_pct_{m}"]    = np.nan
                    rec[f"_lgbest_{m}"] = _lg_bests.get((pk, m), np.nan)
            else:
                rec[f"_pct_{m}"]    = np.nan
                rec[f"_lgbest_{m}"] = np.nan
        ...
        best_role_name, best_role_val = "", -1.0
        for rname, spec in ROLE_BUCKETS.get(rk, {}).items():
            rv, rw = [], []
            for m, w in spec.get("metrics", {}).items():
                p = rec.get(f"_pct_{m}", np.nan)
                if pd.notna(p):
        ...
        base      = float(max(role_vals)) if role_vals else (rec.get("Complete Score") or 0.0)
        ls_norm   = np.clip(float(LEAGUE_STRENGTHS.get(lg, 70.0)) / 100.0, 0.30, 1.00)
        rec["Impact Score"] = float(base * ls_norm ** 1.6)
...
def _make_team_ranking_image(
    ...
    silhouette = _tr_load_silhouette()

    hi_set = {str(x).strip().lower() for x in (highlight_names or [])}
    def is_hi(row): return str(row.get("Player", "")).strip().lower() in hi_set
    ...
    # ── Bar max = league best for the ranked metric ───────────────────────────
    lgbest_col = f"_lgbest_{rank_col}"
    if lgbest_col in df_top.columns:
        lbv = pd.to_numeric(df_top[lgbest_col], errors="coerce").dropna()
        max_bar = float(lbv.iloc[0]) if not lbv.empty else 100.0
    else:
        # Composite/role scores are 0–100 percentile space
        max_bar = 100.0
    if max_bar <= 0:
        max_bar = 100.0

    footer_lines = [
        f"Ranked by: {rank_col.replace('_role_', '')}  ·  Bar = % of league best  ·  Pct ranks vs league+position pool",
    ...
    def _draw_rows(ax,
        ...
        for i, (_, row) in enumerate(df_top.iterrows()):
            ...
            # Rank circle
            ax.scatter([RANK_X], [y], s=rank_dot_s,
                       facecolor=RANK_BG,
                       edgecolor=(HILITE_E if is_hi(row) else RANK_EDGE),
            ...
            if photo is not None:
                zz = min(46.0 / max(photo.shape[0], photo.shape[1], 1), 0.48)
            ...
            ax.text(NAME_X, y + name_dy, player_name,
                    fontsize=name_fs, fontweight="bold",
                    color=TXT, ha="left", va="center", zorder=6)

            # Sub-line: role name when "Best Role" mode, else all positions
            if rank_mode == "Best Role":
                sub_txt   = str(row.get("_best_role_name", "")).strip()
                sub_color = ROLE_C
            else:
                sub_txt   = str(row.get("_all_positions", row.get("Position", "")))
                if not sub_txt.strip():
                    sub_txt = str(row.get("Position", "")).split(",")[0].strip().upper()
                sub_color = SUB

            if show_age and pd.notna(row.get("Age")):
                try:
                    sub_txt += f"  ·  {int(float(row.get('Age', 0)))}y"
                except Exception:
                    pass

            ax.text(NAME_X, y - sub_dy, sub_txt,
                    fontsize=sub_fs, color=sub_color,
                    ha="left", va="center", zorder=6)

            # Bar — fraction of LEAGUE BEST
            v_bar = float(row[rank_col]) if pd.notna(row.get(rank_col)) else 0.0
            frac  = min(1.0, max(0.0, v_bar / max_bar))
            bar_col = _rank_score_color(v_bar) if show_score_pill else "#6b7cff"

            ax.add_patch(Rectangle((BAR_L, y - BAR_H / 2), BAR_R - BAR_L, BAR_H,
            ...
            ax.text(VAL_X, y, _rank_val_fmt(v_disp),
                    ...
                    color=TXT, ha="right", va="center", zorder=6)
    ...
    if export_mode == "1920×1080 (banner)":
        ...
        for k, line in enumerate(footer_lines):
            ax.text(L, ft_y - 0.018 - k * 0.023, line,
                    fontsize=12, color=FOOT, ha="left", va="top")

        row_gap = (hd_y - ft_y - 0.01) / N
        row_h   = row_gap * 0.95
        ROW_TOP = hd_y - 0.015
        ...
        _draw_rows(ax,
                   ...
                   row_gap = row_gap,
                   row_h   = row_h,
                   name_fs = 26, sub_fs = 17, val_fs = 28,
                   name_dy = row_h * 0.20,
                   sub_dy  = row_h * 0.24,
                   rank_dot_s = 1200)
    ...
    _draw_rows(ax,
               ...
               row_gap = row_gap,
               row_h   = row_h,
               name_fs = 15, sub_fs = 10, val_fs = 14,
               name_dy = 0.14,
               sub_dy  = 0.13,
               rank_dot_s = 500)

    ax.plot([0.04, 0.96], [0.85]*2, color=DIV, lw=0.9, zorder=2)
    for k, line in enumerate(footer_lines):
        ax.text(0.04, 0.66 - k * 0.19, line,
    ...
    if _rank_team_players.empty:
        ...
        with _rc1:
            _rank_pos_group = st.selectbox(
                "Position group", list(_RANK_POS_GROUPS.keys()), index=0, key="tr_pos_group",
            )
        with _rc2:
        ...
        if (st.session_state.get("_tr_cache_key") != _rank_cache_key
            ...
            st.session_state.pop("_tr_img_bytes", None)
            with st.spinner("Computing team player scores…"):
                try:
                    _tr_df = _build_team_rank_df(
                        ...
                        team=sel_team, league=t_league, min_mins=_rank_min_mins,
                    )
        ...
        if _tr_df is None or _tr_df.empty:
            ...
            _tr_filtered = (
                _tr_df[_tr_df["_ftok"].isin(_allowed_pos)].copy()
                if _allowed_pos is not None else _tr_df.copy()
            )

            if _tr_filtered.empty:
                st.info(f"No players in position group: {_rank_pos_group}")
            else:
                # All role columns present in filtered DF
                _role_cols = sorted(
                    c for c in _tr_filtered.columns
                    if c.startswith("_role_") and not c.startswith("_lgbest_")
                )

                # Raw metrics for this position group
                ...
                _raw_met_cols  = [m for _, m in _raw_met_pairs if m in _tr_filtered.columns]

                # ── Rank mode radio ───────────────────────────────────────────
                _rk1, _rk2 = st.columns([1.6, 2.4])
                with _rk1:
                    _rank_mode = st.radio(
                        "Rank by",
                        ["Composite score", "Best Role", "Raw metric"],
                        horizontal=True,
                        key="tr_rank_mode",
                    )
                ...
                with _rk2:
                    if _rank_mode == "Composite score":
                        # Complete Score, Impact Score, then individual role cols
                        _comp_opts = ["Complete Score", "Impact Score"] + _role_cols
                        def _pretty_c(c):
                            return c.replace("_role_", "") if c.startswith("_role_") else c
                        _rank_col_sel = st.selectbox(
                            "Metric", _comp_opts, format_func=_pretty_c, key="tr_comp_metric",
                        )
                        _value_col = _rank_col_sel

                    elif _rank_mode == "Best Role":
                        # Rank each player by their personal best role score
                        # The sort column is "_best_role_score"; show role name in image
                        _rank_col_sel = "_best_role_score"
                        _value_col    = "_best_role_score"
                        st.caption("Each player ranked by their highest role score. Role name shown in image.")
                ...
                if _rank_col_sel in _tr_sorted.columns:
                    _tr_sorted[_rank_col_sel] = pd.to_numeric(
                        _tr_sorted[_rank_col_sel], errors="coerce"
                    )
                    _tr_sorted = (
                        _tr_sorted
                        .sort_values(_rank_col_sel, ascending=False)
                        .dropna(subset=[_rank_col_sel])
                ...
                # Default titles
                if _rank_mode == "Best Role":
                    _metric_label = "BEST ROLE SCORE"
                elif _rank_col_sel.startswith("_role_"):
                    _metric_label = _rank_col_sel.replace("_role_", "").upper()
                else:
                    _metric_label = _rank_col_sel.upper()

                _rt1, _rt2, _rt3 = st.columns(3)
                with _rt1:
                    _rank_t1 = st.text_input("Title 1", f"{sel_team.upper()} SQUAD",  key="tr_t1")
                with _rt2:
                    _rank_t2 = st.text_input("Title 2", _metric_label,                key="tr_t2")
                with _rt3:
                    _rank_t3 = st.text_input("Title 3", _rank_pos_group.upper(),       key="tr_t3")

                # Photo / badge helpers
                def _tr_photo_func(row):
                    key_id = (f"{_norm_str(str(row.get('Player', '')))}|"
                              f"{_norm_str(str(row.get('Team', '')))}")
                ...
                with st.expander("📊 Full Metric Breakdown", expanded=False):
                    _show_cols = [
                        "Player", "_all_positions", "Minutes played", "Age",
                        "Complete Score", "Impact Score",
                        "_best_role_name", "_best_role_score",
                    ]
                    _show_cols += _role_cols
                    for _, m in _raw_met_pairs:
                        if m in _tr_sorted.columns:
                            _show_cols.append(m)
                        if f"_pct_{m}" in _tr_sorted.columns:
                            _show_cols.append(f"_pct_{m}")

                    _show_cols = list(dict.fromkeys(
                        c for c in _show_cols if c in _tr_sorted.columns
                    ))
                    _ren = {
                        "_all_positions":   "All Positions",
                        "_best_role_name":  "Best Role",
                        "_best_role_score": "Best Role Score",
                    }
                    _ren.update({c: c.replace("_role_", "🎯 ") for c in _role_cols})
                    _ren.update({f"_pct_{m}": f"Pct: {m}" for _, m in _raw_met_pairs})

                    st.dataframe(
                        _tr_sorted[_show_cols].rename(columns=_ren).reset_index(drop=True),
                        use_container_width=True,
                    )
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    def _rr_polar_bars_fig(labels, percentiles):
        ...
        for i, lab in enumerate(labels):
            ax.text(angles[i], 135, str(lab).upper(),
    ...
    def _rr_split_polar_fig(team_labels, team_pcts, role_labels, role_pcts):
        ...
        for th, w, v in zip(angles, widths, all_vals):
            ...
            ax.bar(th, v, width=w, bottom=0,
                   color=_RR_CMAP(v / 100), edgecolor="white", linewidth=1.4, zorder=3)
        ...
        corner = "#f472b6"
        fig.text(0.04, 0.965, "TEAM",  ha="left",  va="top", fontsize=18, fontweight="900", color=corner)
        fig.text(0.96, 0.965, "ROLE",  ha="right", va="top", fontsize=18, fontweight="900", color=corner)
        return fig

    # ── Role config ────────────────────────────────────────────────────────────────
    def _rr_get_cfg(role_key: str):
        rk = str(role_key).lower().strip()

        if rk == "cb":
            require = ["Aerial duels per 90","Defensive duels per 90","Passes per 90",
                       "Forward passes per 90","Progressive passes per 90",
                       "Progressive runs per 90","PAdj Interceptions","Shots blocked per 90"]
            def pos_ok_cb(s): s = str(s).upper().strip(); return s.startswith(("CB","RCB","LCB"))
            def compute_cb(df):
                out = df.copy()
                out["Pass Verticality"]   = _rr_safe_div(out["Forward passes per 90"], out["Passes per 90"])
                out["Pass Volume"]        = pd.to_numeric(out["Passes per 90"], errors="coerce")
                out["Positional Demand"]  = (pd.to_numeric(out["PAdj Interceptions"], errors="coerce") +
                                              pd.to_numeric(out["Shots blocked per 90"], errors="coerce"))
                out["Defensive Volume"]   = pd.to_numeric(out["Defensive duels per 90"], errors="coerce")
                out["Progression Volume"] = (pd.to_numeric(out["Progressive passes per 90"], errors="coerce") +
                                              pd.to_numeric(out["Progressive runs per 90"], errors="coerce"))
                out["Aerial Volume"]      = pd.to_numeric(out["Aerial duels per 90"], errors="coerce")
                return out
            return dict(pos_filter=pos_ok_cb, require_cols=require, compute_metrics=compute_cb,
                        agg_cols=["Pass Volume","Pass Verticality","Progression Volume",
                                  "Defensive Volume","Positional Demand","Aerial Volume"],
                        title="Center Backs")

        if rk == "fb":
            require = ["Passes per 90","Forward passes per 90","Progressive passes per 90",
            ...
            def pos_ok_fb(s): s = str(s).upper().strip(); return s.startswith(("LB","LWB","RB","RWB"))
            def compute_fb(df):
                out = df.copy()
                out["Pass Verticality"]       = _rr_safe_div(out["Forward passes per 90"], out["Passes per 90"])
                out["Progression Volume"]     = (pd.to_numeric(out["Progressive passes per 90"], errors="coerce") +
                                                  pd.to_numeric(out["Progressive runs per 90"], errors="coerce"))
                out["Ball Carrying"]          = (0.6*pd.to_numeric(out["Dribbles per 90"], errors="coerce") +
                                                  0.4*pd.to_numeric(out["Progressive runs per 90"], errors="coerce"))
                out["Attacking Contribution"] = (0.4*pd.to_numeric(out["xA per 90"], errors="coerce") +
                                                  0.2*pd.to_numeric(out["Crosses per 90"], errors="coerce") +
                                                  0.2*pd.to_numeric(out["Touches in box per 90"], errors="coerce") +
                                                  0.1*pd.to_numeric(out["Shots per 90"], errors="coerce") +
                                                  0.1*pd.to_numeric(out["Passes to penalty area per 90"], errors="coerce"))
                out["Defensive Volume"]       = (0.5*pd.to_numeric(out["Defensive duels per 90"], errors="coerce") +
                                                  0.3*pd.to_numeric(out["PAdj Interceptions"], errors="coerce") +
                                                  0.2*pd.to_numeric(out["Aerial duels per 90"], errors="coerce"))
                out["Pass Volume"]            = pd.to_numeric(out["Passes per 90"], errors="coerce")
                out["Retention"]              = pd.to_numeric(out["Accurate passes, %"], errors="coerce")
                return out
            return dict(pos_filter=pos_ok_fb, require_cols=require, compute_metrics=compute_fb,
                        agg_cols=["Pass Volume","Pass Verticality","Progression Volume",
                                  "Ball Carrying","Attacking Contribution","Defensive Volume","Retention"],
                        title="Fullbacks")

        if rk == "cm":
            require = ["Passes per 90","Forward passes per 90","Progressive passes per 90",
                       "Progressive runs per 90","Defensive duels per 90",
                       "PAdj Interceptions","Touches in box per 90","Shots per 90","Accurate passes, %"]
            def pos_ok_cm(s): s = str(s).upper().strip(); return s.startswith(("DMF","LDMF","RDMF","LCMF","RCMF","CMF"))
            def compute_cm(df):
                out = df.copy()
                out["Pass Verticality"]    = _rr_safe_div(out["Forward passes per 90"], out["Passes per 90"])
                out["Pass Volume"]         = pd.to_numeric(out["Passes per 90"], errors="coerce")
                out["Progression Volume"]  = (pd.to_numeric(out["Progressive passes per 90"], errors="coerce") +
                                               pd.to_numeric(out["Progressive runs per 90"], errors="coerce"))
                out["Defensive Volume"]    = pd.to_numeric(out["Defensive duels per 90"], errors="coerce")
                out["Interception Volume"] = pd.to_numeric(out["PAdj Interceptions"], errors="coerce")
                out["Retention"]           = pd.to_numeric(out["Accurate passes, %"], errors="coerce")
                return out
            return dict(pos_filter=pos_ok_cm, require_cols=require, compute_metrics=compute_cm,
                        agg_cols=["Pass Volume","Pass Verticality","Progression Volume",
                                  "Defensive Volume","Interception Volume","Retention"],
                        title="Central Midfielders")

        if rk == "attack":
            require = ["Accurate passes, %","xG per 90","Non-penalty goals per 90",
            ...
            def compute_att(df):
                ...
                out["Goal Threat"]       = (0.4*pd.to_numeric(out["xG per 90"], errors="coerce") +
                                             0.4*pd.to_numeric(out["Non-penalty goals per 90"], errors="coerce") +
                                             0.2*pd.to_numeric(out["Touches in box per 90"], errors="coerce"))
                out["Creativity Threat"] = (0.65*pd.to_numeric(out["xA per 90"], errors="coerce") +
                                             0.35*pd.to_numeric(out["Passes to penalty area per 90"], errors="coerce"))
                out["Passing Volume"]    = pd.to_numeric(out["Passes per 90"], errors="coerce")
                out["Deeper Playmaking"] = (0.5*pd.to_numeric(out["Progressive passes per 90"], errors="coerce") +
                                             0.5*pd.to_numeric(out["Passes to final third per 90"], errors="coerce"))
                out["Ball Carrying"]     = (0.6*pd.to_numeric(out["Dribbles per 90"], errors="coerce") +
                                             0.4*pd.to_numeric(out["Progressive runs per 90"], errors="coerce"))
                return out
            return dict(pos_filter=pos_ok_att, require_cols=require, compute_metrics=compute_att,
                        agg_cols=["Retention Style","Goal Threat","Creativity Threat",
                                  "Passing Volume","Deeper Playmaking","Ball Carrying"],
                        title="Attackers")

        if rk == "cf":
            require = ["Touches in box per 90","xG per 90","Dribbles per 90",
                       "Progressive runs per 90","Aerial duels per 90","Aerial duels won, %",
                       "Passes per 90","Non-penalty goals per 90","Accurate passes, %"]
            def pos_ok_cf(s): return str(s).upper().strip().startswith("CF")
            def compute_cf(df):
                out = df.copy()
                out["Opportunities"]      = (0.7*pd.to_numeric(out["Touches in box per 90"], errors="coerce") +
                                              0.3*pd.to_numeric(out["xG per 90"], errors="coerce"))
                out["Ball Carrying"]      = (0.65*pd.to_numeric(out["Dribbles per 90"], errors="coerce") +
                                              0.35*pd.to_numeric(out["Progressive runs per 90"], errors="coerce"))
                out["Aerial Requirement"] = (pd.to_numeric(out["Aerial duels per 90"], errors="coerce") *
                                              pd.to_numeric(out["Aerial duels won, %"], errors="coerce") / 100.0)
                out["Passing Volume"]     = pd.to_numeric(out["Passes per 90"], errors="coerce")
                out["Goal Output"]        = pd.to_numeric(out["Non-penalty goals per 90"], errors="coerce")
                out["Retention"]          = pd.to_numeric(out["Accurate passes, %"], errors="coerce")
                return out
            return dict(pos_filter=pos_ok_cf, require_cols=require, compute_metrics=compute_cf,
    ...
    def _rr_compute_role_pcts_team(player_df, league, team, cfg, min_mins):
        ...
        if ldf.empty:
            return None, f"No eligible {cfg['title']} in {league} with ≥{min_mins} mins."
        ldf = cfg["compute_metrics"](ldf)
        agg = ldf.groupby("Team")[cfg["agg_cols"]].mean().reset_index()
        if team not in agg["Team"].values:
    ...
    def _rr_compute_role_pcts_player(player_df, league, team, player_name, cfg, min_mins):
        ...
        ldf = cfg["compute_metrics"](ldf)
        prow = ldf[ldf["Player"].astype(str) == str(player_name)]
        if prow.empty:
            return None, f"'{player_name}' not found in filtered pool."
        pcts = []
        for met in cfg["agg_cols"]:
    ...
    for _rr1_tab, _rr1_rk in zip(_rr1_tabs, _RR_TAB_KEYS):
        with _rr1_tab:
            _rr1_role_cfg = _rr_get_cfg(_rr1_rk)
            if _rr1_role_cfg is None:
    ...
    st.markdown(
        ...
        'Build a custom forecast — set metrics and percentiles, then hit <b>Generate</b>. '
        'Left = Team style &nbsp;·&nbsp; Right = Role profile. Min 4 per side.</p>',
        unsafe_allow_html=True,
    )
    ...
    _PV_ALL_OPTIONS = sorted([
        ...
        "Short Pass Volume", "Shot Volume", "Shots Faced", "Sweeping", "Transition Speed",
        "Vertical Runs", "Width", "xG", "xGA",
    ])

    # ── Session state init (only on first load per role) ──────────────────────────
    for _pv_rk, _pv_cfg in _PV_DEFAULTS.items():
        for _pv_side in ("team", "role"):
            _pv_sk = f"_pv_metrics_{_pv_rk}_{_pv_side}"
            if _pv_sk not in st.session_state:
                st.session_state[_pv_sk] = list(_pv_cfg[_pv_side])

    # ── Helpers ───────────────────────────────────────────────────────────────────
    def _pv_read_vals(rk, side):
        """Read current percentile values from widget state without triggering rerun."""
        metrics = st.session_state[f"_pv_metrics_{rk}_{side}"]
        return [(m, int(st.session_state.get(f"_pv_val_{rk}_{side}_{i}", 50)))
                for i, m in enumerate(metrics)]

    def _pv_render_side(rk, side, color):
        """Render metric list with sliders. Add/remove handled via callbacks — no mid-render rerun."""
        sk = f"_pv_metrics_{rk}_{side}"
        metrics = st.session_state[sk]

        st.markdown(
            f'<p style="color:{color};font-weight:800;font-size:12px;'
            f'letter-spacing:0.08em;margin:6px 0 4px;">{side.upper()}</p>',
            unsafe_allow_html=True,
        )

        # ── Add row ──────────────────────────────────────────────────────────────
        _pv_add_key   = f"_pv_addsel_{rk}_{side}"
        _pv_addbtn_key = f"_pv_addbtn_{rk}_{side}"
        available = ["— add metric —"] + [m for m in _PV_ALL_OPTIONS if m not in metrics]
        ac1, ac2 = st.columns([4, 1])
        with ac1:
            st.selectbox("add", available, key=_pv_add_key, label_visibility="collapsed")
        with ac2:
            def _pv_do_add(rk=rk, side=side):
                sel = st.session_state.get(f"_pv_addsel_{rk}_{side}", "")
                sk2 = f"_pv_metrics_{rk}_{side}"
                if sel and sel != "— add metric —" and sel not in st.session_state[sk2]:
                    st.session_state[sk2].append(sel)
            st.button("＋", key=_pv_addbtn_key, on_click=_pv_do_add,
                      use_container_width=True)

        # ── Per-metric rows ───────────────────────────────────────────────────────
        for i, m in enumerate(list(metrics)):  # snapshot to avoid mutation mid-loop
            c_lbl, c_val, c_rep, c_del = st.columns([2, 1, 2, 0.45])
            with c_lbl:
                st.markdown(
                    f'<p style="color:#e5e7eb;font-size:12px;margin:0;padding-top:10px;">{m}</p>',
                    unsafe_allow_html=True,
                )
            with c_val:
                st.slider(
                    "pct", 0, 100,
                    value=st.session_state.get(f"_pv_val_{rk}_{side}_{i}", 50),
                    key=f"_pv_val_{rk}_{side}_{i}",
                    label_visibility="collapsed",
                )
            with c_rep:
                rep_opts = ["— replace —"] + [x for x in _PV_ALL_OPTIONS if x not in metrics or x == m]
                rep_key  = f"_pv_rep_{rk}_{side}_{i}"
                def _pv_do_replace(rk=rk, side=side, idx=i, old=m):
                    new = st.session_state.get(f"_pv_rep_{rk}_{side}_{idx}", "— replace —")
                    if new and new != "— replace —":
                        sk2 = f"_pv_metrics_{rk}_{side}"
                        lst = st.session_state[sk2]
                        if old in lst:
                            lst[lst.index(old)] = new
                st.selectbox("rep", rep_opts, key=rep_key,
                             on_change=_pv_do_replace,
                             label_visibility="collapsed")
            with c_del:
                if len(metrics) > 4:
                    def _pv_do_remove(rk=rk, side=side, idx=i, name=m):
                        sk2 = f"_pv_metrics_{rk}_{side}"
                        if name in st.session_state[sk2]:
                            st.session_state[sk2].remove(name)
                    st.button("✕", key=f"_pv_rem_{rk}_{side}_{i}",
                              on_click=_pv_do_remove,
                              use_container_width=True)

    # ── Tab rendering ─────────────────────────────────────────────────────────────
    _pv_tabs = st.tabs([_PV_DEFAULTS[k]["title"] for k in _PV_DEFAULTS])

    for _pv_tab, _pv_rk in zip(_pv_tabs, _PV_DEFAULTS.keys()):
        with _pv_tab:
            _pv_cfg  = _PV_DEFAULTS[_pv_rk]
            _pv_left, _pv_right = st.columns([1, 2])

            with _pv_left:
                _pv_render_side(_pv_rk, "team", "#60a5fa")
                st.markdown('<div style="height:10px;"></div>', unsafe_allow_html=True)
                _pv_render_side(_pv_rk, "role", "#f472b6")
                st.markdown('<div style="height:8px;"></div>', unsafe_allow_html=True)
                _pv_label = st.text_input(
                    "Label",
                    value=f"{_pv_cfg['title']} — Projected",
                    key=f"_pv_lbl_{_pv_rk}",
                    label_visibility="visible",
                )
                _pv_generate = st.button(
                    "Generate radar",
                    key=f"_pv_gen_{_pv_rk}",
                    use_container_width=True,
                    type="primary",
                )

            with _pv_right:
                # Store last-rendered chart in session state so it persists across reruns
                _pv_cache_key = f"_pv_chart_{_pv_rk}"

                if _pv_generate:
                    _pv_t_vals = _pv_read_vals(_pv_rk, "team")
                    _pv_r_vals = _pv_read_vals(_pv_rk, "role")
                    _pv_t_labels = [x[0] for x in _pv_t_vals]
                    _pv_t_pcts   = [x[1] for x in _pv_t_vals]
                    _pv_r_labels = [x[0] for x in _pv_r_vals]
                    _pv_r_pcts   = [x[1] for x in _pv_r_vals]

                    if len(_pv_t_labels) < 4 or len(_pv_r_labels) < 4:
                        st.warning("Need at least 4 metrics per side.")
                    else:
                        _pv_fig = _rr_split_polar_fig(_pv_t_labels, _pv_t_pcts,
                                                       _pv_r_labels, _pv_r_pcts)
                        _pv_fig.text(0.5, 0.01, _pv_label,
                                     ha="center", va="bottom", fontsize=10,
                                     color="#9ca3af", style="italic")
                        _pv_buf = _BytesIO_rr()
                        _pv_fig.savefig(_pv_buf, format="png", dpi=300,
                                        bbox_inches="tight",
                                        facecolor=_pv_fig.get_facecolor())
                        _pv_buf.seek(0)
                        st.session_state[_pv_cache_key] = _pv_buf.getvalue()
                        plt.close(_pv_fig)

                if _pv_cache_key in st.session_state:
                    st.image(st.session_state[_pv_cache_key], use_container_width=True)
                    st.download_button(
                        f"⬇️ Download radar",
                        data=st.session_state[_pv_cache_key],
                        file_name=f"projected_{_pv_rk}_{_uuid_rr.uuid4().hex[:6]}.png",
                        mime="image/png",
                        key=f"_pv_dl_{_pv_rk}",
                    )
                else:
                    st.markdown(
                        '<p style="color:#4b5563;font-size:13px;margin-top:60px;text-align:center;">'
                        'Set your values and hit <b>Generate radar</b></p>',
                        unsafe_allow_html=True,
                    )
